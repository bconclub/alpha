...
- **Energy Confirmation Guard**:
  - Tracks consecutive low-energy ticks via `_low_energy_ticks`
  - `OPT_ENERGY_WINNER_FADING` fires only after >= 3 low-energy ticks
  - `OPT_ENERGY_DEAD_LOSER` fires only after >= 3 low-energy ticks and >= 180s hold
