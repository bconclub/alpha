  ...
  const assetCards = useMemo(() => {
    ...
    for (const log of strategyLog) {
      ...
      if (!existing) {
        ...
        // Prefer Delta over Binance for same asset
        latestByAsset.set(asset, log);
      }
      // strategyLog is already ordered by created_at DESC, so first hit is latest
    }

    // Build cards
    const cards: AssetCard[] = [];
    const entries = Array.from(latestByAsset.entries());
    for (const [asset, log] of entries) {
