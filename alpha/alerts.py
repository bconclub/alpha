...
class AlertManager:
    ...
    async def send_startup(
        ...
        if binance_balance is not None:
            balance_lines += f"\n   \U0001f7e1 Binance: <code>{format_usd(binance_balance)}</code>"
        if delta_balance is not None:
            balance_lines += f"\n   \U0001f7e0 Delta: <code>{format_usd(delta_balance)}</code>"
    ...
    async def send_market_update(
        ...
        if binance_rows:
            lines.append("")
            lines.append(f"<b>BINANCE (Spot)</b>")
            lines.append(LINE)
        ...
        if delta_rows:
            lines.append("")
            lines.append(f"<b>DELTA (Futures)</b>")
            lines.append(LINE)
            for a in delta_rows:
                short = _pair_short(a["pair"])
                emoji = _COND_EMOJI.get(a.get("condition", ""), "\u2753")
    ...
    # ── STRATEGY SWITCH (kept for backward compat — prefer send_strategy_changes) ──

    async def send_strategy_switch(
        self, pair: str, old: str | None, new: str | None, reason: str,
    ) -> None:
        """Single-pair strategy switch — kept for backward compat only."""
        # No-op: strategy changes are now batched via send_strategy_changes.
        # This avoids spamming individual messages per pair.
        pass
