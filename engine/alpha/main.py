...
class AlphaBot:
    ...
    def __init__(self) -> None:
        ...
        # Fill reconciliation: dedup set for processed fill IDs ("exchange:fill_id")
    ...
    async def start(self) -> None:
        ...
        self._scheduler.add_job(self._reconcile_fills, "interval", seconds=120)
    ...
    async def _reconcile_fills(self) -> None:
        """Fetch recent fills from exchanges and reconcile with DB.

        Catches fills the bot missed (no DB entry) and corrects wrong exit
        prices on orphan-closed trades.  Runs every 120 seconds.

        For each fill:
        1. Dedup via in-memory set (fast) — skip known fills
        2. Match against DB trades by exchange+pair+side+price+time — skip tracked
        3. If unmatched EXIT fill → close matching open DB trade with real price
        4. If unmatched fill corrects an orphan-closed trade → update exit price
        5. If unmatched ENTRY fill → create new DB trade as open
        """
        if not self.db.is_connected:
            return

        exchanges: list[tuple[str, Any, list[str]]] = []
        if self.kraken and self._kraken_enabled:
            exchanges.append(("kraken", self.kraken, self.kraken_pairs))
        if self.delta and self._delta_enabled:
            exchanges.append(("delta", self.delta, self.delta_pairs))
        if self.bybit and self._bybit_enabled:
            exchanges.append(("bybit", self.bybit, self.bybit_pairs))

        for exchange_id, exchange, pairs in exchanges:
            try:
                await self._reconcile_exchange_fills(exchange_id, exchange, pairs)
            except Exception:
                logger.exception("Fill reconciliation failed (%s)", exchange_id)

    async def _reconcile_exchange_fills(
        self, exchange_id: str, exchange: Any, pairs: list[str],
    ) -> None:
        """Reconcile recent fills for a single exchange against DB."""
        # Fee rates for P&L calculation
        fee_map: dict[str, float] = {
            "kraken": config.kraken.taker_fee,
            "delta": getattr(config.delta, "taker_fee_with_gst", config.delta.taker_fee),
            "bybit": config.bybit.taker_fee,
        }
        fee_rate = fee_map.get(exchange_id, 0.0005)

        leverage_map: dict[str, int] = {
            "kraken": config.kraken.leverage,
            "delta": config.delta.leverage,
            "bybit": config.bybit.leverage,
        }
        default_leverage = leverage_map.get(exchange_id, 20)
        ...
        for pair in pairs:
            # ── Fetch recent fills ───────────────────────────────────────
            try:
                fills = await exchange.fetch_my_trades(pair, limit=20)
            except Exception as e:
                logger.debug("Failed to fetch fills for %s/%s: %s", exchange_id, pair, e)
                continue

            if not fills:
                continue

            # Process fills oldest-first so entry fills precede their exits
            fills.sort(key=lambda f: f.get("timestamp", 0) or 0)
            ...
            for fill in fills:
                ...
                dedup_key = f"{exchange_id}:{fill_id}"
                if dedup_key in self._processed_fill_ids:
                    continue

                fill_time = fill.get("datetime", "") or ""
                fill_side = fill.get("side", "")         # buy / sell
                fill_price = float(fill.get("price", 0) or 0)
                fill_amount = float(fill.get("amount", 0) or 0)

                if fill_price <= 0 or fill_amount <= 0 or not fill_side:
                    self._processed_fill_ids.add(dedup_key)
                    continue

                # ── Check if fill matches an existing DB trade (entry) ───
                try:
                    matched = await self.db.find_trade_by_fill(
                        exchange_id, pair, fill_side, fill_price, fill_time,
                    )
                except Exception:
                    matched = None

                if matched:
                    # Already tracked — stamp exchange_fill_id if missing
                    trade_id = matched.get("id")
                    if trade_id and not matched.get("exchange_fill_id"):
                        try:
                            await self.db.update_trade(
                                trade_id, {"exchange_fill_id": fill_id},
                            )
                        except Exception:
                ...
                # ── Unmatched fill — needs reconciliation ────────────────
                total_unmatched += 1

                # ── Case A: EXIT fill — close matching open DB trade ─────
                try:
                    open_trade = await self.db.get_open_trade(
                        pair=pair, exchange=exchange_id,
                    )
                except Exception:
                    open_trade = None

                if open_trade:
                    db_pos_type = open_trade.get("position_type", "")
                    is_exit = (
                        (db_pos_type == "long" and fill_side == "sell")
                        or (db_pos_type == "short" and fill_side == "buy")
                    )
                    if is_exit:
                        entry_price = float(open_trade.get("entry_price", 0) or 0)
                        leverage = open_trade.get("leverage", default_leverage) or default_leverage
                        amount = float(open_trade.get("amount", 0) or 0)

                        result = calc_pnl(
                            entry_price, fill_price, amount,
                            db_pos_type, leverage,
                            exchange_id, pair,
                        ...
                        if trade_id:
                            update_data: dict[str, Any] = {
                                ...
                                "closed_at": fill_time or iso_now(),
                                "pnl": round(result.net_pnl, 8),
                                "pnl_pct": round(result.pnl_pct, 4),
                                "exit_reason": "FILL_SYNC",
                            }
                            try:
                                update_data["exchange_fill_id"] = fill_id
                                await self.db.update_trade(trade_id, update_data)
                            except Exception:
                                # Retry without exchange_fill_id (column may not exist)
                                update_data.pop("exchange_fill_id", None)
                                await self.db.update_trade(trade_id, update_data)

                            logger.warning(
                                "FILL_SYNC: closed trade id=%s %s/%s exit=$%.4f "
                                "pnl=$%.4f (%.2f%%)",
                                trade_id, pair, exchange_id,
                                fill_price, result.net_pnl, result.pnl_pct,
                            )
                            total_reconciled += 1
                ...
                # ── Case B: Correct orphan-closed trade with real price ──
                try:
                    orphan_trade = await self.db.find_orphan_closed_trade(
                        exchange_id, pair, fill_time,
                    )
                except Exception:
                    orphan_trade = None

                if orphan_trade:
                    db_pos_type = orphan_trade.get("position_type", "")
                    expected_exit_side = "sell" if db_pos_type == "long" else "buy"

                    if fill_side == expected_exit_side:
                        old_exit = float(orphan_trade.get("exit_price", 0) or 0)
                        # Only correct if the price difference is meaningful (> 0.1%)
                        if old_exit > 0 and abs(old_exit - fill_price) / old_exit > 0.001:
                            entry_price = float(orphan_trade.get("entry_price", 0) or 0)
                            leverage = orphan_trade.get("leverage", default_leverage) or default_leverage
                            amount = float(orphan_trade.get("amount", 0) or 0)
                            ...
                            old_reason = orphan_trade.get("exit_reason", "ORPHAN")
                            if orphan_id:
                                corr_data: dict[str, Any] = {
                                    "exit_price": fill_price,
                                    "pnl": round(result.net_pnl, 8),
                                    "pnl_pct": round(result.pnl_pct, 4),
                                    "exit_reason": f"{old_reason}_CORRECTED",
                                }
                                try:
                                    corr_data["exchange_fill_id"] = fill_id
                                    await self.db.update_trade(orphan_id, corr_data)
                                except Exception:
                                    corr_data.pop("exchange_fill_id", None)
                                    await self.db.update_trade(orphan_id, corr_data)

                                old_pnl = float(orphan_trade.get("pnl", 0) or 0)
                                logger.warning(
                                    "FILL_SYNC: corrected orphan id=%s %s/%s: "
                                    "exit $%.4f→$%.4f, pnl $%.4f→$%.4f",
                                    orphan_id, pair, exchange_id,
                                    old_exit, fill_price,
                                    old_pnl, result.net_pnl,
                                )
                                total_reconciled += 1
                ...
                # ── Case C: Untracked ENTRY fill — create new DB trade ───
                # Safety: don't create if a strategy is actively managing this pair
                # (its normal flow will handle the DB insert)
                scalp = self._get_scalp(pair, exchange=exchange_id)
                if scalp and scalp.in_position:
                    self._processed_fill_ids.add(dedup_key)
                    continue

                position_type = "long" if fill_side == "buy" else "short"
                notional = fill_price * fill_amount
                cost = notional / default_leverage if default_leverage > 1 else notional
                ...
                trade_data: dict[str, Any] = {
                    ...
                    "entry_price": fill_price,
                    "amount": fill_amount,
                    "contracts": fill_amount,
                    "cost": round(cost, 8),
                    "collateral": round(cost, 8),
                    "strategy": "scalp",
                    "order_type": "market",
                    "exchange": exchange_id,
                    "status": "open",
                    "opened_at": fill_time or iso_now(),
                    "reason": "FILL_SYNC (untracked entry)",
                    "leverage": default_leverage,
                    "position_type": position_type,
                    "setup_type": "fill_sync",
                }
                try:
                    trade_data["exchange_fill_id"] = fill_id
                    new_id = await self.db.log_trade(trade_data)
                except Exception:
                    # Retry without exchange_fill_id column
                    trade_data.pop("exchange_fill_id", None)
                    new_id = await self.db.log_trade(trade_data)

                logger.warning(
                    "FILL_SYNC: created trade id=%s %s/%s %s entry=$%.4f × %.4f",
                    new_id, pair, exchange_id, position_type,
                    fill_price, fill_amount,
                )
                total_reconciled += 1
                self._processed_fill_ids.add(dedup_key)

        if total_unmatched > 0 or total_reconciled > 0:
            logger.info(
                "FILL_SYNC: %s found %d unmatched fills, reconciled %d",
                exchange_id, total_unmatched, total_reconciled,
            )
