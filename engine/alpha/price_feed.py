...
Delta India only: raw aiohttp WS to wss://socket.india.delta.exchange
(ccxt.pro doesn't support Delta)

Architecture:
- PriceFeed runs as a background asyncio task (Delta WS)
- On every price update: calls strategy.check_exits_immediate(price)
...
class PriceFeed:
    """Real-time price feed via Delta WebSocket for instant exit checks.

    Usage:
        feed = PriceFeed(strategies, delta_pairs=["BTC/USD:USD", "ETH/USD:USD"])
        await feed.start()
        ...
        await feed.stop()
    """
    ...
    async def start(self) -> None:
        """Start Delta WS feed as background task."""
        self._running = True
        logger.info("PriceFeed starting — Delta: %d pairs", len(self._delta_pairs))
    ...
    def register_wake_callback(self, pair: str, callback: object) -> None:
        """No-op — momentum wake disabled in Delta-only mode."""
        pass

    def _on_price_update(self, pair: str, price: float) -> None:
        """Handle a price update — update cache, check exits."""
        if price <= 0:
            return
        ...
        # Call strategy on every tick: exits when in position
        # Strategy dict uses exchange-prefixed keys ("delta:BTC/USD:USD")
        strategy = self._strategies.get(f"delta:{pair}")
        if strategy:
            self._exit_checks += 1
            try:
            ...
            if msg_type == "v2/ticker":
                ...
                if symbol and price_str:
                    ...
                    if pair:
                        ...
                        self._on_price_update(pair, price)
                    return
